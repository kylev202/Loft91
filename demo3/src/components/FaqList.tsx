    ...
    <div className="max-w-(--container-measure)">
      {items.map(({ q, a }) => (
        <details key={q} className="group border-b border-rule" data-reveal>
          <summary className="flex min-h-14 cursor-pointer list-none items-center justify-between gap-md py-sm text-item text-ink transition-colors duration-(--dur-micro) ease-out hover:text-brass [&::-webkit-details-marker]:hidden">
            <span className="text-pretty">{q}</span>
            <span
              aria-hidden="true"
              className="shrink-0 text-lead text-brass transition-transform duration-(--dur-short) ease-out group-open:rotate-45"
            >
              +
            </span>
          </summary>
