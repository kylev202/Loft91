...
- **WebGL** (all browsers): shader effects, post-processing, particle systems. Libraries: Three.js, OGL (lightweight), regl. Use for effects CSS can't express.
  - **react-three-fiber** (`@react-three/fiber`): declarative React renderer for Three.js — build WebGL scenes as component trees with hooks. No performance overhead vs vanilla Three.js. `npm i three @types/three @react-three/fiber`
  - **@shadergradient/react**: 3D animated shader gradient renderer. Hero backgrounds and ambient surfaces that respond to camera controls; mesh types: plane, sphere, waterPlane. `npm i @shadergradient/react @react-three/fiber three`
  - **liquid-glass-js**: Apple Liquid Glass-inspired real-time refraction and blur for web. Vanilla JS `Container` and `Button` classes; no bundler required. Nested glass where child elements sample parent output.
  - **liquid-logo** (standalone tool, no install): browser-based tool that transforms logos into animated liquid-metal WebGL graphics. Reference implementation for edge-detection + fluid-dynamics + metallic-sheen shader pipelines. [Live demo](https://collidingscopes.github.io/liquid-logo)
